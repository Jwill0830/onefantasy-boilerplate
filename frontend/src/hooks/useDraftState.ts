...
import { League, Team, LeagueSettings } from '../types';
import apiClient from '../services/api';
import { useAuth } from './useAuth';
import { useSocket } from './useSocket';

// Define missing types locally until they're added to types file
...
export const useLeagueData = (leagueId?: string): UseLeagueDataReturn => {
  ...
  // Computed values
  const isCommissioner = state.league?.commissioner_id === currentUser?.uid;
  const userTeam = state.teams.find(team => team.owner_id === currentUser?.uid) || null;
  ...
  }, [leagueId, currentUser, refreshLeagueData]);
